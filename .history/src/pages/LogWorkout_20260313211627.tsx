...
const LogWorkout = () => {
  ...
  const getExerciseUnit = (exerciseId: string): WeightUnit => currentWorkout.exerciseUnits[exerciseId] ?? unitPreference;
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {splitExercises.map((exercise) => {
            ...
            const inputMode = getExerciseInputMode(exercise.name);
            return (
                ...
                <CardContent className="space-y-2">
                  ...
                  {inputMode !== 'reps_weight' && (
                    <div className="grid grid-cols-[2rem_1fr_2rem] gap-2 text-[10px] text-muted-foreground font-medium">
                      <span>Set</span>
                      <span>{inputMode === 'duration' ? 'Duration (sec)' : 'Reps'}</span>
                      <span></span>
                    </div>
                  )}
                  {sets.map((s, i) => (
                    <motion.div
                      ...
                      className={cn(
                        'gap-2 items-center',
                        inputMode === 'reps_weight' ? 'grid grid-cols-[2rem_1fr_1fr_2rem]' : 'grid grid-cols-[2rem_1fr_2rem]',
                      )}
                    >
